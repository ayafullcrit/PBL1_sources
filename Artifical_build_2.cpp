...
void PrintSimplexTable()
{
    ...
    FOR(i, 1, M + 1)
    {
        ...
        cout << endl; //<< LMS;
        cout << x << setw(10) << VariableRow[i] << x << setw(10);
        FOR(j, 1, N + TTS + 1)
        {
...
void TwoPhase()
{
    ...
    SubRow.resize(MAX + 1);   
	FOR(i,1,M+1)
	FOR(j,1,N+TotalSubVariable+1) f[i][j]=0;
...
void NotValid()
{
    ...
    if (HitKey())
    {
        NhapTuBanPhim(); // quay lai nhap lai du lieu
    }
    return;
}
            ...
            else
            {
                ...
                else
                {
                    ...
                    else // ton tai ki tu 'x'
                    {
                        ...
                        f[i][stoi(VariableIndex)] = PrepSign * stoi(Factor);
